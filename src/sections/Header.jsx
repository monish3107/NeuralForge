...
const [hasScrolled, setHasScrolled] = useState(false);
    ...
    const NavLink = ({ title }) => {
        return (
            <LinkScroll onClick={()=>setIsOpen(false)} to={title} offset={-100} smooth spy activeClass="nav-active" className="base-bold text-p4 uppercase transition-colors duration-500 cursor-pointer hover:text-p1 max-lg:my-4 max-lg:h-5">
                {title}
            </LinkScroll>
        );
    }
                            ...
                            <ul className="flex max-lg:block max-lg:px-12">
                                ...
                                <li className="nav-logo">
                                    <LinkScroll to="hero" offset={-250} spy smooth className={clsx("max-lg:hidden transition-transform duration-500 cursor-pointer")}>
                                        <img src="/images/xora.svg" alt="xora" width={160} height={155}/>
                                    </LinkScroll>
                                </li>
